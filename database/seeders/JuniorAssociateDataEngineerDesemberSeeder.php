...
class JuniorAssociateDataEngineerDesemberSeeder extends Seeder
{
    /**
    ...
    {
        // Ambil data skema Junior / Associate Data Engineer
        $skema = Skema::where('kode', 'JADE')->first();

        // Ambil TUK FIKLAB-401
        $tuk = Tuk::where('kode', 'TUK13')->first();

        if (!$skema || !$tuk) {
            $this->command->error('Skema Junior / Associate Data Engineer atau TUK tidak ditemukan!');
            return;
        }
        ...
        $jadwal = Jadwal::create([
            ...
            'status' => 4, // Selesai
            'kuota' => 2,
            'keterangan' => 'Ujian Sertifikasi Junior / Associate Data Engineer - 2 Desember 2024',
        ]);

        // Ambil 2 asesi
        $asesiEmails = [
            'aji.supriyatno@gmail.com',
            'abimanyu.priyatno@gmail.com',
        ];
        ...
        // Ambil asesor
        $asesor1 = User::where('email', 'wayan.widi@asesor.com')->first();
        $asesor2 = User::where('email', 'nindy.irzavika@asesor.com')->first();
        ...
        $asesorDistribution = [
            $asesor1->id => [$asesiList[0]], // I Wayan Widi Pradnyana - Aji Supriyatno
            $asesor2->id => [$asesiList[1]], // Nindy Irzavika - Abimanyu Priyatno
        ];
        ...
        foreach ($asesorDistribution as $asesorId => $asesiGroup) {
            foreach ($asesiGroup as $asesi) {
                ...
                $asesiPenilaian = AsesiPenilaian::create([
                    ...
                    'catatan_asesor' => 'Asesi menunjukkan pemahaman yang baik dan mampu menerapkan konsep Junior / Associate Data Engineer dengan tepat',
        ...
        $this->command->info('');
        $this->command->info('========================================');
        $this->command->info('Seeder Junior / Associate Data Engineer Desember 2024 selesai!');
        $this->command->info('========================================');

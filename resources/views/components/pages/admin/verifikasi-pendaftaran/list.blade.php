                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center align-items-center" style="gap: 0.5rem;">
                                        ...
                                        @if ($item->status == 5)
                                            <form action="{{ route(auth()->user()->user_type . '.verifikasi-pendaftaran.update', $item->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body bg-light">
                    ...
                    <!-- Formulir APL Asesi Card -->
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-gradient-warning text-white">
                            <h6 class="mb-0 font-weight-bold">
                                <i class="fas fa-file-alt mr-2"></i>Formulir APL Asesi
                            </h6>
                        </div>
                        <div class="card-body">
                            <div class="alert alert-info mb-3" role="alert">
                                <i class="fas fa-info-circle mr-2"></i>
                                <strong>Petunjuk:</strong> Klik tombol di bawah untuk download Formulir APL 1 dan APL 2 (DOCX) yang telah diisi oleh Asesi.
                            </div>
                            <div class="d-flex justify-content-center gap-3" style="gap: 1rem;">
                                <a id="btn-apl1" href="#" class="btn btn-primary">
                                    <i class="fas fa-download mr-2"></i>Download APL 1
                                </a>
                                <a id="btn-apl2" href="#" class="btn btn-warning">
                                    <i class="fas fa-download mr-2"></i>Download APL 2
                                </a>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.keteranganButton').on('click', function (event) {
                    ...
                    var routeName = userType + '.verifikasi-pendaftaran.update';
                    var updateRoute = '{{ route("kaprodi.verifikasi-pendaftaran.update", ":id") }}'.replace('kaprodi', userType).replace(':id', id);
                    modal.find('#formKeterangan').attr('action', updateRoute);
                ...
                $('.viewDetailsButton').on('click', function (event) {
                    ...
                    // Update APL buttons URL for PDF preview
                    var pendaftaranId = item.id;
                    var userType = '{{ auth()->user()->user_type }}';
                    var apl1Url = '/' + userType + '/verifikasi-pendaftaran/' + pendaftaranId + '/preview-apl1';
                    var apl2Url = '/' + userType + '/verifikasi-pendaftaran/' + pendaftaranId + '/preview-apl2';
                    
                    $('#btn-apl1').attr('href', apl1Url);
                    $('#btn-apl2').attr('href', apl2Url);
                });
